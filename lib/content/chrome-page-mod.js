...
const { frames, process } = require('sdk/remote/child');
 ...
 * @constructor
 */
const ChildChromeMod = Class({
  implements: [
    EventTarget,
    Disposable,
  ],
  setup: function ChromeMod(model) {
    merge(this, model);
...
frames.addEventListener("DOMWindowCreated", onContentWindow, true);
...
function createWorker(mod, window) {
  ...
  frame.port.emit('chrome-page-mod/worker-create', mod.id, {
    id: workerId,
...
process.port.on('chrome-page-mod/create', (process, model) => {
  if (pagemods.has(model.id))
    return;

  new ChildChromeMod(model);
});

process.port.on('chrome-page-mod/destroy', (process, id) => {
