...
const { WorkerHost, connect } = require('sdk/content/utils');
...
const { remove, has, hasAny } = require("sdk/util/array");
const { Rules } = require("sdk/util/rules");
const { processes, frames, remoteRequire } = require('sdk/remote/parent');
remoteRequire(module.uri.replace("chrome-page-mod.js", "content/chrome-page-mod.js"));
...
const ChromeMod = Class({
  ...
  setup: function ChromeMod(options) {
    ...
    processes.port.emit('chrome-page-mod/create', model.childOptions);
  },

  dispose: function(reason) {
    processes.port.emit('chrome-page-mod/destroy', modelFor(this).id);
...
processes.forEvery(process => {
  for (let mod of pagemods.values())
    process.port.emit('chrome-page-mod/create', modelFor(mod).childOptions);
});

frames.port.on('chrome-page-mod/worker-create', (frame, modId, workerOptions) => {
