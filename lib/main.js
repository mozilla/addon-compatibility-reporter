...
var reporterPanel = panel.Panel({
    ...
    contentScriptFile: [self.data.url("reporter.js"), self.data.url("jquery-1.7.2.min.js")]
});
    ...
    for (var i=0; i<addonReports.length; i++) {
        var makeCB = function() {
            ...
            return function(response) {
                ...
                addonReports[ix].state = addonReports[ix].newstate;
        ...
        if (!addonReports[i].hasSubmitted) {
            if (addonReports[i].newstate == 1 || addonReports[i].newstate == 2) {
                ACR.Logger.log("about to submit report for: " + addonReports[i].guid);
                ACR.submitReport(addonReports[i],
                    (addonReports[i].newstate == 1),
...
var widget = widgets.Widget({
    ...
    contentURL: self.data.url("image/acr.png"),
    panel: reporterPanel
});
