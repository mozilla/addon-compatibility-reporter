...
var reporterPanel = panel.Panel({
    width: (ACR.Util.getHostEnvironmentInfo().osName=="WINNT"?390:380),
...
var widget = widgets.Widget({
    ...
    contentURL: self.data.url("image/extensionGeneric-Darwin.png"),
    panel: reporterPanel,
    onAttach: function() {
      ...
      timers.setTimeout(function() { widget.contentURL = self.data.url("image/extensionGeneric-16-"+os+".png"); }, 100);
    }
});
...
var showWidgetPanel = function() {
    ...
    var widget = win.document.getElementById("widget:"+self.id+"-acr-dialog");
    if (widget) {
        widget.parentNode.collapsed = false;
