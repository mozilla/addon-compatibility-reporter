...
try {
    ChromeMod({
        ...
        onAttach: function(worker) {
            worker.port.emit("acr_init", {
                exclamationImageURL: self.data.url("image/exclamation.png"),
                informationImageURL: self.data.url("image/information.png"),
                warningImageURL: self.data.url("image/warning.png"),
                appE10sEnabled: ACR.Util.getHostEnvironmentInfo().multiprocessEnabled
            });
...
var reporterPanel = panel.Panel({
    ...
    contentScriptFile: [self.data.url("reporter.js"),
        self.data.url("lib/jquery-3.1.0.min.js"),
        self.data.url("lib/jquery-ui-1.12.0.min.js")],
    onHide: function() {
        if (widget.state)
...
function retrieveAddonIcon(addon, callback) {
    if (!addon.iconURL) {
        ...
        try {
            ...
            // If anything goes wrong (for example an invalid iconURL) fall back to the generic icon.
            addon.icon = genericAddonIconURL;
            callback(addon);
        }
    }
            ...
            if (addon.type === "extension" && !addon.isSystem && addon.isActive) {
                ...
                retrieveAddonIcon(addon, function callback(addon) {
                    ...
                    if (reportsProcessed === addonsCount) {
                        ...
                        addonReports.sort((a, b) => a.name.localeCompare(b.name));
...
events.on("acr_reset_reporter_panel", function() {
    reporterPanel.port.emit("reset");
}, true);
